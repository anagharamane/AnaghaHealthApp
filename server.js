...
var plotly = require('plotly')('harsha1403','ON9MqTS4pmtlfNBoZPHI');
...
// var initData = [{x:[], y:[], stream:{token:'8pj5c3ooqa', maxpoints:200}}];
// var initGraphOptions = {fileopt : "extend", filename : "Ankita More ECG",layout:layout};
...
router.post("/sendECGData",function(req,res){
	console.log("Comming in sendECGData!!!!")
...
// });
	...
	plotly.plot(data, {layout: layout}, function (err, msg) {
  		...
  		console.log(msg);
  		plotly_url = msg.url
  // 		var stream1 = plotly.stream('8pj5c3ooqa', function (err, res) {
		//    // console.log("Comming hereeee....")
		//     console.log(err, res);
		//     var streamObject = JSON.stringify({ x : req.body.x, y : req.body.y});
  //     		stream1.write(streamObject+'\n');
		//     // clearInterval(loop); // once stream is closed, stop writing 
		// });
  		Patient.findOneAndUpdate({_id: req.body.patient_id}, {$set:{plotly_url: plotly_url}}, {new: true}, function(err, patient){
			console.log("in find one and update")
			console.log("Patient is "+patient)
		  if(err){
		      console.log("Something wrong when updating patient for plotly url!");
		      res.json({
		      	success:false,
